...
void fPartBinNumDec(double num) // вывод дробной части числа в двоичном виде
{
    int arr[32];
    int use = 0;
    int r;
...
int mul10(int* arr) // умножение двоичного числа на 1010
{
    ...
    arr1[1] = 0;
    arr1[2] = 0;
    for (i = 3; i < 131; i++) arr1[i] = arr[i - 3];
    arr1[131] = 0;
    arr2[0] = 0;
    for (i = 1; i < 129; i++) arr2[i] = arr[i - 1];
    for (i = 129; i < 132; i++) arr2[i] = 0;
...
{
    int i;
    for (i = 0; i < 128; i++) if (arr[i] != 0) return 0;
    return 1;
...
void translation(unsigned long long num, long long p) // перевод из двоичного вида в десятичный
{
    ...
    for (i = 0; i < 64; i++) arr[i] = num << i >> 63;
    for (i = 64; i < 128; i++) arr[i] = 0;
...
int main()
{
    ...
    printf("\n\nFloat part in dec: ");
    translation(fPartNum(M(u.i), C(u.i)), C(u.i));
    // fPartBinNumDec(u.f);
    // printf("\n\nInteger part in dec: %d", S(u.i) == 1 ? -1 * iPartNum(M(u.i), C(u.i)) : iPartNum(M(u.i), C(u.i)));
    // целое в десятичном
...
// printf("\n\nFloat part in dec:%c0.", S(u.i) == 1 ? '-' : ' '); translation(fPartNum(M(u.i), C(u.i)), C(u.i)); // дробное в десятичном
